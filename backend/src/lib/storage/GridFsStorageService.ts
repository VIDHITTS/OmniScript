...
export class GridFsStorageService implements StorageService {
  ...
  upload(stream: Readable, metadata: FileMetadata): Promise<string> {
    return new Promise((resolve, reject) => {
      if (!this.bucket) return reject(new Error("GridFS Bucket not initialized. Call connect() first."));
      ...
      stream.pipe(uploadStream)
        .on('error', (err) => {
          logger.error({ err, filename: metadata.filename }, 'Error uploading file to GridFS');
          reject(err);
        })
        .on('finish', () => {
  ...
  async download(fileId: string): Promise<Readable> {
    ...
    try {
      ...
      logger.error({ fileId, err }, 'Error opening download stream from GridFS');
      throw err;
    }
  }
